...
### Preset 架构（Phase 0–4 新增）

```
src/config/presets/
├── builtin.js                    # 内置预设注册表（8 个预设，含 features 能力标记）
└── index.js                      # Preset 入口：查询/CRUD/能力标记规范化/模型派生

src/core/services/
└── modelFetcher.js               # 从远端 {baseUrl}/models 拉取模型列表

src/modes/DrawMode/
├── index.vue                     # 绘图模式（读 preset.features.imageOutput）
└── plugin.js                     # 绘图模式插件配置

src/appCore/methods/
└── configMethods.js              # Preset 切换/CRUD 副作用处理
```
...
```
docs/api/
├── Preset架构改造方案.md         # Phase 0–4 改造方案（含进度追踪清单）
└── API架构梳理.md                # 已更新为反映 Preset 架构
...
### src/utils/
所有工具函数保持不变：
- ✅ keyManager.js（已升级：按 presetId 分桶）
- ✅ markdown.js
- ✅ pdfExporter.js
- ✅ txtExporter.js
- ✅ scriptPreview.js
- ✅ archive.js
- ✅ providers/openaiCompatible.js（原 deepseek.js，Phase 1A 改名）
- ✅ providers/gemini.js
