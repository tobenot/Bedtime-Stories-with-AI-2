...
function generateNestedUtilities(utilities, prefix, values, builder, e) {
	Object.entries(values).forEach(([key, value]) => {
		if (typeof value === 'string') {
			const name = `${prefix}-${key}`;
			utilities[`.${e(name)}`] = builder(value);
			return;
		}
		generateNestedUtilities(utilities, `${prefix}-${key}`, value, builder, e);
	});
}

const scrollbarPlugin = plugin(function({ addUtilities, theme, variants, e }) {
	const utilities = {
		'.scrollbar': {
	...
	generateNestedUtilities(
		...
		theme('colors'),
		(value) => ({
			'&::-webkit-scrollbar-track': { backgroundColor: value }
		}),
		e
	);

	addUtilities(utilities, variants('scrollbar'));
});
...
module.exports = {
	...
	plugins: [
		...
		plugin(function({ addComponents, theme }) {
			const iconButton = {
				'.btn-refresh, .btn-copy, .btn-delete, .btn-edit, .btn-fork, .btn-collapse, .btn-cache': {
					display: 'inline-flex',
					alignItems: 'center',
					justifyContent: 'center',
